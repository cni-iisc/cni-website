...
Dr Pannag R Sanketi, a Tech-Lead Manager in the Robotics team at Google Deepmind (previously called Google Brain) in Mountain View, California, provided an overview of robotics research at Google DeepMind, highlighting their goal of building a robot-agnostic robotics formation model that can work with different types of robots. They aim to scale up the robot, model size, compute power, and data collection to solve complex tasks and create an open-source dataset for smaller labs to fine-tune models. He discussed the process of labelling mistakes and picking data, the use of simulation, and the challenges and benefits of reinforcement learning. He also delve into specific projects such as navigation, personality robotics, agile robotics, and self-improvement. Furthermore, he explored the use of language models for high-level task planning, training robots to perform tasks independently, and incorporating robot manipulation data into large models. 

## SaFeR – A Safety Framework for e-Scooter Riders
Mahima Agumbe Suresh, an Assistant Professor at San Jose State University,  presented her research on a safety framework for e-scooter riders, inspired by her personal experience of almost getting hit by a vehicle. The framework aims to provide situational awareness to e-scooter riders through Advanced Driver Assistance algorithms, similar to those used in cars, to inform them about potential obstacles and collisions in real-time. The speaker discussed different methods for perceiving depth using camera systems, such as using monocular cameras and utilizing depth networks and pose networks. She also explored optimization techniques for deep learning networks, including channel pruning and quantization, to improve the efficiency of object detection and recognition algorithms. The SaFeR framework for e-scooter riders is introduced, which reduces the complexity of the network and improves inference time while maintaining performance. The importance of integrating this safety framework into a broader matrix and collaborating with experts in perception and optimal control is emphasized. The speaker provided insights into the challenges and potential solutions for enhancing the safety of e-scooter riders, including early evasive action, compliance with city regulations, and embedding safety features in e-scooters.

## The Robustness of Blockchains to Network Delays
Suryanarayana Sankagiri (Surya) ,a postdoctoral researcher in the Information and Network Dynamics Lab at EPFL, Switzerland  discussed the robustness of blockchains to network delays in this video. He explored the security guarantees of Bitcoin and the challenges in providing formal security analysis. Dr. Sankagiri questioned the realistic nature of bounded communication delays and proposes a network model with random and unbounded delays. He also highlighted the importance of confirmation processes and the vulnerabilities that network delays can pose to blockchains. Additionally, he discussed the mathematical assumptions needed to prove the robustness of blockchains to delays and suggested relaxing network assumptions to achieve similar results. Overall, Dr. Sankagiri provided insights into the security analysis and network conditions of blockchains.
